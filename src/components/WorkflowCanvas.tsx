...
export const WorkflowCanvas: React.FC<WorkflowCanvasProps> = ({
  ...
  // Workflow Email Alerts & Metrics state
  const [showWorkflowMetricsModal, setShowWorkflowMetricsModal] = useState<boolean>(false);
  const isWorkflowEmailActive = workflow.emailNotificationsEnabled !== false;

  const handleToggleWorkflowEmail = () => {
    const nextVal = !isWorkflowEmailActive;
  ...
  const handleSimulatePipelineRun = () => {
    ...
    const updated: Workflow = {
      ...
      totalEmailsSent: (workflow.totalEmailsSent || 0) + (isWorkflowEmailActive ? 1 : 0),
      lastRun: "Just now",
    ...
    <div id="workflow-studio-root" className="flex-1 flex flex-col h-[calc(100vh-4rem)] overflow-hidden bg-slate-100 dark:bg-slate-950 select-none">
      ...
      <div className="h-14 border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 px-4 flex items-center justify-between z-20 shrink-0">
        ...
        <div className="flex items-center gap-2">
          ...
          {/* Email Notification Alerts Toggle for this Pipeline */}
          <button
            id="btn-toggle-workflow-email-notifications"
            type="button"
            onClick={handleToggleWorkflowEmail}
            className={`hidden lg:flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg border text-xs font-semibold transition-all ${
              isWorkflowEmailActive
            ...
            title={isWorkflowEmailActive ? "Email Alerts Active: Dispatches execution receipts (Click to mute)" : "Email Alerts Muted for this pipeline (Click to activate)"}
          >
            <Mail className={`w-3.5 h-3.5 ${isWorkflowEmailActive ? "text-indigo-600 dark:text-indigo-400" : "text-slate-400"}`} />
            <span className="hidden xl:inline">Email Alerts:</span>
            <span className={`text-[10px] font-bold ${isWorkflowEmailActive ? "text-indigo-600 dark:text-indigo-400" : "text-slate-400"}`}>
              {isWorkflowEmailActive ? "ON" : "OFF"}
            </span>
          </button>

          {/* Workflow Metrics Button */}
          <button
            id="btn-open-workflow-metrics-modal"
            type="button"
            onClick={() => setShowWorkflowMetricsModal(true)}
            className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 text-xs font-semibold transition-colors"
            title="Inspect Pipeline Metrics, Telemetry & Email History"
          >
            <BarChart3 className="w-3.5 h-3.5 text-indigo-500" />
            <span className="hidden sm:inline">Metrics</span>
            <span className="text-[10px] font-mono px-1 rounded bg-slate-100 dark:bg-slate-700 font-bold">
                      ...
                      <div className="flex items-center gap-3 shrink-0">
                        <div className="text-right hidden sm:block">
                          <span className="text-xs font-bold text-slate-800 dark:text-slate-200 block">
                            {wf.totalRuns || 0} runs • {wf.totalEmailsSent || 0} emails
                          </span>
                          <span className="text-[9px] text-emerald-600 dark:text-emerald-400 font-semibold block">
                            ~{((wf.totalRuns || 0) * (wf.avgHoursSavedPerRun || 0.5)).toFixed(1)}h automated • {wf.successRate ?? 99.4}% success
                          </span>
                        </div>

                        {/* Email Alert Toggle Button */}
                        <button
                          type="button"
                          onClick={(e) => {
                            e.stopPropagation();
                            const nextVal = !(wf.emailNotificationsEnabled ?? true);
                            playInteractiveSound(nextVal ? "chime" : "click");
                            onSaveWorkflow({
                              ...wf,
                              emailNotificationsEnabled: nextVal
                          ...
                          className={`hidden md:flex items-center gap-1 px-2.5 py-1.5 rounded-xl text-[11px] font-bold border transition-all ${
                            wf.emailNotificationsEnabled !== false
                          ...
                          title="Toggle automated email notifications for this workflow"
                        >
                          <Mail className="w-3 h-3" />
                          <span>{wf.emailNotificationsEnabled !== false ? "Alerts: ON" : "Alerts: OFF"}</span>
                        </button>
      ...
      {/* WORKFLOW METRICS & EMAIL NOTIFICATION MODAL */}
      <WorkflowMetricsModal
        isOpen={showWorkflowMetricsModal}
        onClose={() => setShowWorkflowMetricsModal(false)}
        workflow={{
          ...workflow,
          name: workflowName,
          description: workflowDesc,
          nodes,
          connections,
          emailNotificationsEnabled: isWorkflowEmailActive
        }}
        onToggleEmailNotifications={handleToggleWorkflowEmail}
        onSimulateRun={handleSimulatePipelineRun}
      />
    </div>
  );
};
