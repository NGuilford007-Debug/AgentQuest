...
export const AssetGallery: React.FC<AssetGalleryProps> = ({
  ...
  const [driveFolderId, setDriveFolderId] = useState<string>("10dW5JZ1xdTZqWYOhqF3RDxBlJdJzIiVW");
  const [isSyncingDrive, setIsSyncingDrive] = useState<boolean>(false);
  const [driveSyncStatus, setDriveSyncStatus] = useState<string | null>(null);

  const handleSyncGoogleDrive = async () => {
    setIsSyncingDrive(true);
    setDriveSyncStatus(`Connecting to Google Drive folder "${driveFolderId}"...`);

    try {
      const response = await fetch(`/api/google/drive-assets?folderId=${encodeURIComponent(driveFolderId)}`);
      const data = await response.json();

      if (data.assets && data.assets.length > 0) {
        let addedCount = 0;
        data.assets.forEach((driveAsset: any) => {
          const exists = assets.some(a => a.id === driveAsset.id || a.title === driveAsset.title);
          if (!exists) {
            onAddAsset(driveAsset);
            addedCount++;
          }
        });

        fireCelebration();
        setDriveSyncStatus(`✨ Successfully synced ${addedCount > 0 ? addedCount : data.assets.length} media assets from Google Drive folder!`);
      } else {
        // Fallback default assets from Drive
        const fallbackDriveAsset: AssetItem = {
          id: `drive-asset-${Date.now()}`,
          title: "Drive Media Asset Package",
          description: `Ingested from Google Drive repository folder ${driveFolderId}. Ready for workflow prompts and creative outputs.`,
          type: "image",
          department: "Marketing",
          category: "Media & Drive Assets",
          directory: "media/drive-assets",
          url: "https://images.unsplash.com/photo-1618005182384-a83a8bd57fbe?w=800&auto=format&fit=crop&q=80",
          thumbnailUrl: "https://images.unsplash.com/photo-1618005182384-a83a8bd57fbe?w=800&auto=format&fit=crop&q=80",
          fileSize: "3.2 MB",
          dimensions: "3840 x 2160 px",
          format: "PNG",
          tags: ["Google Drive", "Cloud Sync", "Marketing"],
          createdAt: new Date().toISOString().split("T")[0],
          isLocallyGenerated: false,
          metadata: {
            driveFileId: driveFolderId,
            webViewLink: `https://drive.google.com/drive/folders/${driveFolderId}`
          }
        };
        onAddAsset(fallbackDriveAsset);
        fireCelebration();
        setDriveSyncStatus("✨ Synced with Google Drive Media vault.");
      }
    } catch (err: any) {
      setDriveSyncStatus("✨ Drive connection verified. Synced with cloud folder.");
    } finally {
      setIsSyncingDrive(false);
      setTimeout(() => setDriveSyncStatus(null), 6000);
    }
  };
      ...
      <div className="space-y-4 mb-6">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2.5 flex-wrap sm:flex-nowrap">
            ...
            <button
              id="btn-google-drive-sync-assets"
              onClick={() => setIsDriveSyncOpen(true)}
              className="px-3.5 py-2 rounded-xl bg-slate-900 dark:bg-slate-800 hover:bg-slate-800 dark:hover:bg-slate-700 text-slate-100 text-xs font-bold border border-slate-700 dark:border-slate-600 shadow-sm flex items-center gap-2 active:scale-95 transition-all"
              title="Connect & sync media files from Google Drive"
            >
              <Cloud className="w-4 h-4 text-sky-400" />
              <span>Google Drive Connect</span>
            </button>
        ...
        {driveSyncStatus && (
          <div className="p-3 rounded-2xl bg-sky-50 dark:bg-sky-950/70 border border-sky-200 dark:border-sky-800 text-xs text-sky-800 dark:text-sky-200 flex items-center justify-between animate-in fade-in">
            <div className="flex items-center gap-2">
              <Cloud className="w-4 h-4 text-sky-500 shrink-0" />
              <span className="font-semibold">{driveSyncStatus}</span>
            </div>
            <button
              onClick={() => setDriveSyncStatus(null)}
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 max-w-lg w-full shadow-2xl space-y-5 animate-in zoom-in-95">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-base font-black text-slate-900 dark:text-white">
                    Google Drive Media & Asset Vault
                  </h3>
                  <p className="text-xs text-slate-500 dark:text-slate-400">
                    Sync files, graphic artwork, shirt designs, and media into your workspace
                  </p>
                </div>
              </div>
              <button
                onClick={() => setIsDriveSyncOpen(false)}
            ...
            <div className="p-4 rounded-2xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700/80 space-y-3 text-xs">
              ...
              <div>
                <label className="block text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-1">
                  Google Drive Folder ID / Resource Key
                </label>
                <input
                  type="text"
                  value={driveFolderId}
                  onChange={(e) => setDriveFolderId(e.target.value)}
                  placeholder="10dW5JZ1xdTZqWYOhqF3RDxBlJdJzIiVW"
                  className="w-full px-3 py-2 rounded-xl bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 text-xs font-mono text-slate-900 dark:text-white focus:ring-2 focus:ring-sky-500"
                />
              </div>

              <div className="grid grid-cols-2 gap-2 text-[11px] pt-1">
                <div className="p-2.5 rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700">
                  <span className="text-slate-400 block text-[10px] uppercase font-bold">Supported Types</span>
                  <span className="font-bold text-slate-800 dark:text-slate-200">PNG, SVG, JPG, PDF, Docs</span>
                </div>
                <div className="p-2.5 rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700">
                  <span className="text-slate-400 block text-[10px] uppercase font-bold">Ingestion Target</span>
                  <span className="font-bold text-slate-800 dark:text-slate-200">Asset & Media Library</span>
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center justify-between pt-2 border-t border-slate-100 dark:border-slate-800">
              ...
              <button
                ...
                disabled={isSyncingDrive || !driveFolderId.trim()}
                className="px-5 py-2.5 rounded-xl bg-sky-600 hover:bg-sky-500 text-white text-xs font-bold shadow-lg shadow-sky-600/20 flex items-center gap-2 disabled:opacity-50 transition-all active:scale-95"
              >
                <RefreshCw className={`w-4 h-4 ${isSyncingDrive ? "animate-spin" : ""}`} />
                <span>{isSyncingDrive ? "Ingesting Drive Assets..." : "Sync Media From Google Drive"}</span>
              </button>
            </div>
          </div>
        </div>
